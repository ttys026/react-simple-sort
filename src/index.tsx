...
export const Sortable: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    document.addEventListener('dragover', (e) => {
      if (!draggingEle.current || !lastEnterElement.current || !lastEnterRect.current || draggingIndex.current === -1) {
        return;
      }
      e.preventDefault();
      const getMousePosition = () => (direction === 'horizontal' ? e.clientX : e.clientY);
      const { left, width, top, height } = lastEnterRect.current;
      const getElementPos = () => (direction === 'horizontal' ? left + width / 2 : top + height / 2);
      if (getMousePosition() > getElementPos()) {
        lastEnterElement.current.insertAdjacentElement('afterend', draggingEle.current!);
      } else {
        lastEnterElement.current.insertAdjacentElement('beforebegin', draggingEle.current!);
      }
    });
  }, []);
          ...
          return React.cloneElement(child, {
            ...
            onDragEnd: () => {
              if (draggingEle.current) {
                ...
                if (parent) {
                  const newIndex = Array.prototype.indexOf.call(parent.children, draggingEle.current);
